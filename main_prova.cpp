...
int main(int argc, char **argv)
{
	...
	feature_extraction_proto_file.push_back("/data/giulia/REPOS/caffe_batch_finetuning/imagenet_val.prototxt");
	...
	out_dir.push_back("/data/giulia/prova");

	// Names of layers to be extracted
	string extract_features_blob_names = "fc6";
	int num_features = 1;
	...
	for (int m=0; m<pretrained_binary_proto_file.size(); m++) {
		...
		for (int batch_index = 0; batch_index < num_mini_batches; batch_index++) {
			...
			for (int i=0; i<batch_size; i++) {
				...
				std::vector<float> codingVec;
				cout << caffe_extractor->extract_singleFeat_1D(img, codingVec) << endl;
			}
